...
"""Refuse stage skips and rasterize without sibling sha / published USGS stage."""
...
def _schema() -> StateSchema:
    return StateSchema.from_specs(
        [
            ...
            ChannelSpec("usgs_stage_pinned", last_value, default=False),
            ChannelSpec("usgs_rasterized", last_value, default=False),
            ChannelSpec("tables_written", last_value, default=False),
...
def _evaluate(state: dict[str, Any]) -> dict[str, Any]:
    ...
    if tr >= _rank("A") and not bool(state.get("usgs_stage_pinned")):
        violations.append("rasterize_without_usgs_stage")
    if tr >= _rank("B") and not bool(state.get("usgs_rasterized")):
        violations.append("compare_without_usgs_raster")
    if tr >= _rank("C"):
        if not bool(state.get("tables_written")):
            violations.append("figure_without_tables")
...
def build_graph() -> StateGraph:
    g = StateGraph(_schema(), name="fim.stage_gate")
